...
(defgeneric discard-burn-in (sample &optional burn-in-fraction)
  (:documentation "Discard the burn-in from the sample.")
  (:method ((sample vector)
            &optional (burn-in-fraction *default-burn-in-fraction*))
    (subseq sample (calculate-burn-in (length sample) burn-in-fraction)))
  (:method ((sample mosaic-matrix)
            &optional (burn-in-fraction *default-burn-in-fraction*))
    (sub sample
         (cons 0 (calculate-burn-in (nrow sample) burn-in-fraction)) t)))
